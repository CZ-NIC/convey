...
class ActionController:
    ...
    def add_aggregation(
        self,
        fn_name: str = None,
        column_task: Optional[str] = None,
        group: Optional[Field] = None,
        grouping_probably_wanted=True,
        exit_on_fail=False,
    ):
        ...
        if column_task:
            field: Field = self.parser.fields[
                self.parser.identifier.get_column_i(
                    column_task, "to be aggregated with"
                )
            ]
        else:
    ...
    def add_dialect(self):
        ...
        while True:
            s = (
                "What is delimiter "
                + (f"(default '{dialect.delimiter}')" if dialect.delimiter else "")
                + ": "
            )
            ...
            s = (
                ...
                + (f"(default '{dialect.quotechar}')" if dialect.quotechar else "")
                + ": "
            )
    ...
    def add_merge(
        self,
        remote_path=None,
        remote_col_i: Optional[int] = None,
        local_col_i: Optional[int] = None,
    ):
        if not remote_path:
            remote_path = self.m.ask(
                "What file we should merge the columns from?", PathTag(is_file=True)
            )
        ...
        if remote_col_i is not None:
            ...
            column2 = controller2.select_col(
                "Select remote column to be merged",
                include_computables=False,
                highlighted=high,
            )
        if not column1:
            high = self.parser.get_similar(column2)
            column1 = self.select_col(
                f"Select local column to merge '{column2}' to",
                include_computables=False,
                highlighted=high,
            )
    ...
    def add_new_column(self, task, add=True):
        ...
        if not self.source_new_column(
            target_type, add, source_field, source_type, custom
        ):
            print("Cancelled")
            exit()
    ...
    def _add_new_column(self, task):
        ...
        try:
            target_type = types[
                types.index(target_type)
            ]  # determine FIELD by exact name
        except ValueError:
            d = {
                t.name: SequenceMatcher(None, task[0], t.name).ratio()
                for t in Types.get_computable_types()
            }
            rather = max(d, key=d.get)
            logger.error(f"Unknown field '{task[0]}', did not you mean '{rather}'?")
            exit()
        source_field, source_type, c = self.parser.identifier.get_fitting_source(
            target_type, *task[1:]
        )
        custom = c + custom
    ...
    def assure_aggregation_group_by(
        self, fn, field, group, grouping_probably_wanted=True, exit_on_fail=False
    ) -> Optional[Field]:
        match (fn == Aggregate.count, group is None, grouping_probably_wanted):
            ...
            case False, True, True:
                ...
                group = self.select_col(
                    "group by",
                    prepended_field=("no grouping", "aggregate whole column"),
                )
            case True, False, _:
                if field != group:
                    logger.error(
                        f"Count column '{field.name}' must be the same"
    ...
    def get_aggregation_fn(
        self, fn_name: str = None, exit_on_fail=False
    ) -> AggregateMethod:
        if not fn_name:
    ...
    def select_col(
        self,
        dialog_title="",
        only_computables=False,
        include_computables=True,
        add=None,
        prepended_field=None,
        highlighted: Optional[List[Field]] = None,
    ) -> Optional[Field]:
        ...
        if include_computables:
            for type_ in Types.get_computable_types():
                if type_.from_message:
                    ...
                    s = (
                        type_.group.name + " "
                        if type_.group != TypeGroup.general
                        else ""
                    )
    ...
    def source_new_column(
        ...
        if not source_field or not source_type:
            ...
            source_field = self.m.select(
                ...
                tips=[
                    ...
                    for i in self.parser.identifier.get_fitting_source_i(target_type)
                ],
            )
            source_col_i = self.parser.fields.index(source_field)
            source_type = self.parser.identifier.get_fitting_type(
                source_field, target_type, try_plaintext=True
            )
            ...
            if source_type is None:
                ...
                if choices := {
                    ...
                    self.m.alert(
                        f"No known method for making {target_type}. Raise your usecase as"
            ...
            try:
                if target_type.group == TypeGroup.custom:
                    if target_type == Types.code:
                        ...
                        *custom, target_type = Preview(
                            source_field, source_type, target_type
                        ).reg()
                    elif (
                        target_type == Types.external
                    ):  # choose a file with a needed method
                        while True:
                            path = self.m.form(
                                {
                                    "What .py file should be used as custom source?": PathTag(
                                        is_file=True
                                    )
                                }
                            )
                            ...
                            if module:
                                ...
                                method_name = self.m.select(
                                    [x for x in dir(module) if not x.startswith("_")],
                                    title=f"What method should be used in the file {path}?",
                                )
                                custom = path, method_name
                                break
                            else:
                                self.m.alert(
                                    f"The file {path} does not exist or is not a valid .py file."
                                )
                    if not custom:
                        raise Cancelled("... cancelled")
            except Cancelled:
            ...
            for i in range(len(path) - 1):
                ...
                if isinstance(m, PickBase):
                    ...
                    if self.m.env.cli.yes:
                        ...
                        m: PickMethod
                        c = self.m.select(
                            {(k, v): k for k, v in m.get_options()}, f"Choose subtype"
                        )
                    elif type(m) is PickInput:
                        m: PickInput
                        c = Preview(source_field, source_type, target_type).pick_input(
                            m
                        )
                    custom.insert(0, c)
        if add is None:
            if self.m.confirm(
                f"New field added: {target_type}\n\nDo you want to include this field as a new column?"
            ):
                add = True
        ...
        if f.source_field.merged_from:
            hit_any_key(
                "Unfortunately, sourcing from columns being merged was not implemented."
