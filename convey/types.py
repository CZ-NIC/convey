...
class Type:
    ...
    def __init__(
        self,
        name,
        group=TypeGroup.general,
        description="",
        usual_names=[],
        identify_method=None,
        is_private=False,
        from_message=None,
        usual_must_match=False,
    ):
        """
        :param name: Key name
    ...
    def __add__(
        self, other
    ):  # sometimes we might get compared to string columns names from the CSV
        if isinstance(other, str):
            return self.name + " " + other
        return self.name + " " + other.name
    ...
    def init(self):
        ...
        computing_start = [
            stop
            for start, stop in methods
            if start is self and methods[start, stop] is True
        ]
        equals = [
            start
            for start, stop in methods
            if stop is self and methods[start, stop] is True
        ]
        if len(computing_start) == 1:
            self.computing_start = computing_start[0]
        elif len(computing_start):
            raise ConnectionAbortedError(
                f"Multiple 'computing_start' types defined for {self}: {computing_start}"
            )
        if equals:
        ...
        if self != Types.plaintext:
            self.is_plaintext_derivable = bool(
                graph.dijkstra(self, start=Types.plaintext)
            )
...
class Types:
    ...
    def refresh(init=False):
        ...
        [
            ...
            for to, from_ in methods
            if methods[to, from_] is not True
        ]
    ...
    def import_method(module, method_name, path, name=None):
        ...
        if isinstance(
            lambda_, ABCMeta
        ):  # this is just an import statement of ex: PickBase
            return
        doc = (
            lambda_.__doc__
            if not isinstance(lambda_, PickBase)
            else lambda_.get_type_description()
        )
    ...
    def find_type(source_type_candidate):
        source_type = None
        source_t = source_type_candidate.lower().replace(
            " ", ""
        )  # make it seem like a usual field name
        possible = None
        for t in types:
    ...
    def get_method(start: Type, target: Type):
        try:
            ...
            if (start, target) in methods_deleted:
                print(
                    f"Disabled path at {start} – {target}. Launch --config to enable it."
                )
                exit()
            else:
                raise LookupError
    ...
    whoisdomain = Type(
        "whoisdomain", TypeGroup.whois, "ask whois servers for domain", is_private=True
    )
    web = Type("web", TypeGroup.web, "scrape web contents", is_private=True)

    external = Type(
        "external", TypeGroup.custom, from_message="from a method in your .py file"
    )
    ...
    reg_s = Type(
        "reg_s", TypeGroup.custom, from_message="substitution from a regular expression"
    )
    reg_m = Type(
        "reg_m", TypeGroup.custom, from_message="match from a regular expression"
    )
    netname = Type("netname", TypeGroup.whois)
    country = Type("country", TypeGroup.whois)
    registrar_abusemail = Type(
        "registrar_abusemail", TypeGroup.whois, "Abuse e-mail contact from whois"
    )
    ...
    http_status = Type(
        "http_status",
        TypeGroup.web,
        "HTTP response status. If 0 or negative, request failed.",
    )
    ...
    country_name = Type(
        "country_name", TypeGroup.general
    )  # XX not identifiable, user has to be told somehow there is such method
    phone = Type(
        "phone",
        TypeGroup.general,
        "telephone number",
    ...
    timestamp = Type(
        "timestamp", TypeGroup.general, "time or date", ["time"], Checker.is_timestamp
    )
    ip = Type(
        "ip", TypeGroup.general, "valid IP address", ["ip", "ipaddress"], Checker.is_ip
    )
    source_ip = Type(
        "source_ip",
        TypeGroup.general,
        "valid source IP address",
        ["sourceipaddress", "source", "src"],
        Checker.is_ip,
    ...
    destination_ip = Type(
        ...
        ["destinationipaddress", "destination", "dest", "dst"],
        Checker.is_ip,
        usual_must_match=True,
    )
    port = Type(
        ...
        "port",
        ["port", "prt"],
        lambda x: re.match(r"\d{1,5}", x),
        usual_must_match=True,
    )
    cidr = Type(
        "cidr", TypeGroup.general, "CIDR 127.0.0.1/32", ["cidr"], Checker.check_cidr
    )
    port_ip = Type(
        "port_ip",
        TypeGroup.general,
        "IP in the form 1.2.3.4.port",
        [],
        reIpWithPort.match,
    )
    any_ip = Type(
        "any_ip",
        TypeGroup.general,
        "IP in the form 'any text 1.2.3.4 any text'",
        [],
    ...
    url = Type(
        ...
        ["url", "uri", "location"],
        lambda s: reUrl.match(s) and "[.]" not in s,
    ...
    quoted_printable = Type(
        "quoted_printable",
        TypeGroup.general,
        "Text encoded as quotedprintable",
        [],
        Checker.is_quopri,
    )
    urlencode = Type(
        "urlencode",
        TypeGroup.general,
        "Text encoded with urlencode",
        ["urlencode"],
        Checker.is_urlencode,
    )
    wrong_url = Type(
        "wrong_url", TypeGroup.general, "Deactivated URL", [], Checker.check_wrong_url
    )
    plaintext = Type(
        "plaintext",
        TypeGroup.general,
    ...
    def get_computable_types(ignore_custom=False) -> list[Type]:
        ...
        for (_, target_type), m in methods.items():
            if ignore_custom and (
                target_type.group == TypeGroup.custom
                or (isinstance(m, PickBase) and not m.default)
            ):
                continue
    ...
    def get_guessable_types() -> list[Type]:
        ...
        return sorted(
            [
                t
                for t in types
                if not t.is_disabled and (t.identify_method or t.usual_names)
            ]
        )
    ...
    def get_uml(flags):
        ...
        used = set()
        formatting_disabled = (
            "[color=lightgray fontcolor=lightgray]" if flags & 1 else ""
        )
        loop = []
        existing_edges = set()
        ...
        for enabled, ((start, target), m) in [(True, f) for f in methods.items()] + [
            (False, f) for f in methods_deleted.items()
        ]:
        ...
        for enabled, start, target, m in loop:
            ...
            if (
                start.group != target.group
                and target.group.name != target
                and target.group in [TypeGroup.dns, TypeGroup.nmap, TypeGroup.custom]
            ):
            ...
            if add(start, target, disable_s):
                ...
                if isinstance(m, PickMethod):
                    l[-1] += (
                        '[label="choose '
                        + ",".join([name for name, _ in m.get_options()])
                        + '" fontsize=10]'
                    )
        ...
        if flags & 16:  # looks nicer in a presentation
            ...
            l.extend(
                ["port -> urlencode[style=invis]", "charset -> timestamp[style=invis]"]
            )
        l.append("}")
        return "\n".join(l)
    ...
    def _get_methods(config: Union["Env", _AlwaysFalse]):
        ...
        return {
            ...
            (
                t.any_ip,
                t.ip,
            ): any_ip_ip,  # "91.222.204.175 93.171.205.34" -> "91.222.204.175" OR '"1.2.3.4"' -> 1.2.3.4
            (
                t.port_ip,
                t.ip,
            ): port_ip_ip,  # IP written with a port 91.222.204.175.23 -> 91.222.204.175
            (t.port_ip, t.port): port_ip_port,
            (t.url, t.hostname): url_hostname,
            (t.url, t.port): url_port,
            (t.hostname, t.ip): (
                Checker.hostname_ips
                if config.comp.multiple_hostname_ip
                else Checker.hostname_ip
            ),
            ...
            (t.cidr, t.ip): (
                Checker.cidr_ips
                if config.comp.multiple_cidr_ip
                else lambda x: str(ipaddress.ip_interface(x).ip)
            ),
            ...
            (t.whois, t.csirt_contact): lambda x: (
                Contacts.country2mail[x.get[5]]
                if x.get[5] in Contacts.country2mail
                else ""
            ),
            (t.whois, t.incident_contact): lambda x: x.get[2],
            (t.abusemail, t.cc_contact): lambda x: "; ".join(
                Contacts.mail2cc[domain]
                for domain in Contacts.get_domains(x)
            ...
            (t.country, t.csirt_contact): lambda x: (
                Contacts.country2mail[x] if x in Contacts.country2mail else ""
            ),
