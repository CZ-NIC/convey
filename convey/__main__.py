...
if sys.version_info[0:2] < (3, 10):
    print(
        "We need at least Python 3.10, your version is "
        + sys.version
...
def main():
    ...
    try:
        ...
        if daemonize_on_exit:
            ...
            try:
                if Config.get_env().process.daemonize and not daemon_pid():
                    ...
                    subprocess.Popen(
                        [sys.argv[0], "--daemon", "server"], stdout=subprocess.DEVNULL
                    )
        ...
        if debug:
            ...
            print(
                f"Convey crashed at {value} on {traceback.format_exc().splitlines()[-3].strip()}"
            )
            if Config.get_env().cli.github_crash_submit:
