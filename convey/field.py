...
class Field:
    def __init__(
        ...
        source_type=None,
        new_custom=None,
        merged_from: Optional[Field] = None,
        merge_operation: Optional[MergeAction] = None,
    ):
        ...
        if source_field:
            ...
            self.source_type = (
                source_type
                if type(source_type) is Type
                else getattr(Types, source_type)
            )
    ...
    def get(self, long=False, color=True, line_chosen=True):
        ...
        if color:
            s = "\n".join(
                (self.color(c, line_chosen=line_chosen) for c in s.split("\n"))
            )
        return s
    ...
    def get_samples(self, max_samples=inf, supposed_type=None, target_type=None):
        ...
        try:
            res = [self.parser.sample_parsed[line][self.col_i] for line in range(0, c)]
        except IndexError:
            rows = []
        ...
        if supposed_type and supposed_type.is_plaintext_derivable:
            ...
            for c in rows:
                for m in self.get_methods(
                    Types.bytes if target_type == Types.charset else Types.plaintext,
                    self.type,
                ):
                    c = m(c)
                res.append(c)
    ...
    def compute_preview(self, source_line: List[Cell]) -> Cell:
        ...
        for _ in range(
            self.col_i - len(source_line) + 1
        ):  # source_line is shorter than we need - fill the missing cols with Nones
