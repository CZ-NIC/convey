...
class Whois:
    ...
    def init(csv):
        Whois.stats = csv.whoisStats
        Whois.ranges = csv.ranges
        Whois.ipSeen = csv.whoisIpSeen # ipSeen[ip] = prefix
    ...
    def _loadCountry(self):
        ...
        for server in list(self.servers):
            ...
            if country:
                ...
                if country[0:4].lower() == "wide": #ex: 'EU # Country is really world wide' (the last word) (64.9.241.202) (Our mirror returned this result sometimes)
                    country = ""
                    continue
    ...
    def getAbusemail(self, forceload=False):
        ...
        for grep in [('% abuse contact for'), ('orgabuseemail'), ('abuse-mailbox')]:
            match = self.reAbuse.search(self._grepResponse(grep)) # the last whois query was the most successful, it fetched country so that it may have abusemail inside as well
            if match:
