...
class Controller:
    ...
    def extendColumn(self, newField, add=None):
        ...
        if method is None:
            ...
            choices = [(k, self.csv.guesses.getDescription(k)) for k, _ in
                       sorted(self.csv.guesses.getGraph().dijkstra(newField, ignore_private=True).items(),
            ...
            if choices:
                title = "Choose the right method\n\nNo known method for making {} from column {} because the column type wasn't identified. How should I treat the column?".format(
                    newField, self.csv.fields[sourceColI])
                button, method = dialog.menu(title, choices=choices)
                if button == "cancel":
                    return
            else:
                dialog.msgbox("No known method for making {}. Raise your usecase as an issue at {}.".format(newField,
                                                                                                            Config.PROJECT_SITE))

        self.csv.settings["add"].append((newField, sourceColI, method))
        self.csv.fields.append(newField)
